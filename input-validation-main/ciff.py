    ...
    def parse_ciff_file(file_path):
        ...
        try:
            with open(file_path, "rb") as ciff_file:
                ...
                if len(magic) != 4:
                    raise Exception("Magic bites must be 4 bytes")
                bytes_read += 4
                ...
                if len(width) != 8:
                    raise Exception("Width is not 8 bytes")
                bytes_read += 8
                # interpret the bytes as an 8-byte-long integer
                # HINT: check out the "q" format specifier!
                # HINT: Does it fit our purposes?
                ...
                if len(height) != 8:
                    raise Exception("Height is not 8 bytes")
                bytes_read += 8
                # interpret the bytes as an 8-byte-long integer
                # HINT: check out the "q" format specifier!
                # HINT: Does it fit our purposes?
                ...
                if len(c) != 1:
                    raise Exception("Character contains more that 1 byte")
                bytes_read += 1
                char = c.decode('ascii')
                ...
                while char != '\n':
                    ...
                    # TODO: check if c contains 1 byte
                    if len(c) != 1:
                        raise Exception("Character in first loop contains more that 1 byte")
                    bytes_read += 1
                    char = c.decode('ascii')
                ...
                while bytes_read != new_ciff.header_size:
                    ...
                    # TODO: check if c contains 1 byte
                    if len(c) != 1:
                        raise Exception("Character in second loop contains more that 1 byte")
                    bytes_read += 1
                    char = c.decode('ascii')
                    # tags should not contain '\n'
                    # TODO: char must not be a '\n'
                    if char == '\n':
                        raise Exception("Char must not be \\n")
                    ...
                    if (bytes_read == new_ciff.header_size) and char != '\0':
                        raise Exception("The last character is not \\0")
                
                # all tags must end with '\0'
                # TODO: check the end of each tag for the '\0'
                for tag in tags:
                    if tag[-1] != "\0":
                        raise Exception("The last character of a tag is not \\0")
                ...
                while bytes_read < new_ciff.header_size+new_ciff.content_size:
                    ...
                    if len(c) != 3:
                        raise Exception("C must contain 3 bytes")
                    bytes_read += 3
                ...
                c = ciff_file.read(1)
                if len(c) == 1:
                    raise Exception("Did not reach the end of file")
